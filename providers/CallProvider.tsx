...
export default function CallProvider({ children }: PropsWithChildren) {
  ...
  return(
    ...
    {children}
    {call && !isOnCallScreen &&
    (<Pressable onPress={()=> router.push(`/call`)} style={{position: 'absolute', backgroundColor: 'lightgreen', top: top + 40, left: 0, right: 0, padding: 10}}>
      <Text>Call :{call?.id}({call.state.callingState})
      </Text>
    </Pressable>
    )}
    </>
